...
from dask_flood_mapper.processing import extract_orbit_names, post_process_eodc_cube, post_process_eodc_cube_, post_processing
from dask_flood_mapper.calculation import calc_water_likelihood, harmonic_expected_backscatter, bayesian_flood_decision
...
class PostProcessing:
    def test_post_processing(self, mock_hpar_dataset):
        ...
        expected_decision = np.logical_and(mock_hpar_dataset.MPLIA >= 27, mock_hpar_dataset.MPLIA <= 48) \
                            * (mock_hpar_dataset.hbsc > (mock_hpar_dataset.wbsc + 0.5 * 2.754041)) \
                            * ((mock_hpar_dataset.sig0 > (mock_hpar_dataset.hbsc - 3 * mock_hpar_dataset.STD)) & 
                               (mock_hpar_dataset.sig0 < (mock_hpar_dataset.hbsc + 3 * mock_hpar_dataset.STD))) \
...
def test_process_sig0_dc(mock_extract_orbit_names, mock_post_process, mock_data, mock_items_orbits):
...
@patch("dask_flood_mapper.processing.post_process_eodc_cube")
@patch("dask_flood_mapper.processing.extract_orbit_names")
def test_process_datacube(mock_extract_orbit_names, mock_post_process, mock_data, mock_items_orbits):
